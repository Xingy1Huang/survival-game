...
class Bomb(pygame.sprite.Sprite):
    def __init__(self, image, scale, x, y, speed):
        ...
        # Randomly pick one direction when the bomb spawns
        self.direction = random.choice(["horizontal", "vertical"])  
        self.movement = random.choice([-1, 1])  # Either move in positive or negative direction
    ...
    def update(self):
        ...
        # Wrap around the screen if bomb reaches the boundaries
        if self.rect.x > screen_width:
            self.rect.x = 0
        if self.rect.y > screen_height:
            self.rect.y = 0
...
background = pygame.image.load('images/explosion.png')


house_img = pygame.image.load("images/pause.jpg")#pause
play_img = pygame.image.load("images/play.jpg")
pause_img = pygame.image.load("images/stop.jpg") #quit
restart_img = pygame.image.load("images/restart.jpg")
level_img = pygame.image.load("images/next level.jpg")
bomb_img = pygame.image.load("images/bomb.jpg")
play_btn = Button(play_img, 0.05, 150+250, 250+250)
pause_btn = Button(pause_img, 0.05, 350+250, 250+250)
house_btn = Button(house_img, 0.05, 970, 35)
restart_btn = Button(restart_img, 0.08, 900, 40)
level_btn = Button(level_img, 0.08, 400, 500)




hamster_img = pygame.image.load("images/mouse.jpg")
...
def next_level():
    ...
    if level_btn.draw():
        ...
        if level == 1:
            ...
            enemy_speed = (int(level)*1.5)
            player_speed = int(level)*2.5
            enemy_count = 1
            bomb_count = int(level)-3
        ...
        for _ in range(bomb_count):
            ...
            bomb = Bomb(bomb_img, 0.05, bomb_x, bomb_y, enemy_speed)
            bomb_group.add(bomb)
...
while True:
    ...
    if game_state == "menu":
        ...
        if restart_btn.draw():
          ...
          # Reset enemies, bombs, and superpowers
          enemy_group.empty()
          bomb_group.empty()
          superpower_group.empty()
